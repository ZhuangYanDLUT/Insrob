...
// std::string strPointcloudTopicName = "/cloud_pcd";//接收激光传感器原始消息
//std::string strPointcloudTopicName = "/velodyne_points";//接收激光传感器原始消息
...
void PoseCallback(nav_msgs::Odometry pose)
{
	...
	//cout<<"yaw: "<<yaw<<" ----> "<<yaw*180/3.1415926<<endl;
...
int main(int argc, char** argv)
{
	...
	*g_pPoseSubscriber = node.subscribe(strPoseTopicName, 100, &PoseCallback);

	/*订阅激光消息*/
 	g_pPointcloudSubscriber = new ros::Subscriber;
    *g_pPointcloudSubscriber = node.subscribe(strPointcloudTopicName, 100, &PointcloudCallback);

 	/*发布地图消息*/   
 	g_pMap_publisher = new ros::Publisher;
	*g_pMap_publisher = node.advertise<nav_msgs::OccupancyGrid>(strPubMapTopicName, 20);
