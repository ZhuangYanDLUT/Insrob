...
void speed_change(CmdType cmd){
	...
	if ((lastlinear + lastlinear_add) <= 3 && (lastlinear + lastlinear_add) >= -3) {
		lastlinear += lastlinear_add;
	}
	if ((lasttwist + lasttwist_add) <= 2 && (lasttwist + lasttwist_add) >= -2) {
		lasttwist += lasttwist_add;
	}
	speed_msg.linear = 100 * lastlinear;
	speed_msg.twist = 100 * lasttwist;
...
int main(int argc, char **argv){
	...
	ros::NodeHandle n;
	speed_pub = n.advertise<Insrob_server::Speed>("/speed_control", 2);
