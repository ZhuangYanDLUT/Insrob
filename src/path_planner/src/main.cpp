...
std::string strPoseTopicName = "/air_pose";
		...
		else
		{
			...
			else
			{
				...
				if(Bspline_Mtrajfinder_.pathID>=Bspline_Mtrajfinder_.CLnum)
				{
					...
					if(CmdtoRob.twist<0)
					{
						temtwist = -1 * CmdtoRob.twist * 180 / 3.1415926;
					}
					else
					{
						temtwist = CmdtoRob.twist * 180 / 3.1415926;
					}

					Base_control::ControlCmd srv;
					srv.request.xx = 52.7774 * CmdtoRob.linear * CmdtoRob.linear * CmdtoRob.linear  
						                 - 169.1789 * CmdtoRob.linear * CmdtoRob.linear + 356.5043 * CmdtoRob.linear
			                 			 + 11.1851;
					srv.request.yy = -0.1217 * temtwist * temtwist * temtwist + 5.1135 * temtwist * temtwist 
						             - 30.3849 * temtwist + 44.0964;

				    srv.request.xx = srv.request.xx * linearratio;

					if(CmdtoRob.twist<0)
					{
						srv.request.yy = srv.request.yy * -1;
					}	    
					...
					/*if(Bspline_Mtrajfinder_.flagbsplinesmooth)
					{
						CPoint2d Subgoal = Bspline_Mtrajfinder_.getgoalpoint(Sugoalps);
					...
					if(Bspline_Mtrajfinder_.flagbazersmooth)
					{
						Cmd_level CmdtoRob;
						
						Bspline_Mtrajfinder_.CoverRCmd(Sugoalps,CmdtoRob);
...
/////////////////////////////// just for test ////////////////////////////////
	...
	// global_path_.GetAlltravelpath(3,1);
 	
	// std::vector<CPoint2d> temallpath;
	// for(int i=1;i<allpath.size();i++)
	// {
	// 	temallpath.push_back(allpath[i-1]);
	// 	float th = atan2((allpath[i].y - allpath[i-1].y),(allpath[i].x - allpath[i-1].x));
	// 	float dis = allpath[i].Dist(allpath[i-1]);

	// 	CPoint2d newpoint;
	// 	newpoint.x = allpath[i-1].x + dis/3*cos(th);
	// 	newpoint.y = allpath[i-1].y + dis/3*sin(th);
	// 	temallpath.push_back(newpoint);

	// 	CPoint2d newpoint1;
	// 	newpoint1.x = allpath[i-1].x + 2*dis/3*cos(th);
	// 	newpoint1.y = allpath[i-1].y + 2*dis/3*sin(th);
	// 	temallpath.push_back(newpoint1);

	// }

	// temallpath.push_back(allpath[allpath.size()-1]);



	// Bspline LBSpline;

	// LBSpline.ShapePoints = temallpath;

	// // for(int j=0;j<allpath.size();j++)
	// // {
		
	// // 	std::cout<<"allpath "<<allpath[j].x<<" "<<allpath[j].y<<std::endl;
	// // }


	// // LBSpline BSpline(temallpath,2);

	// std::vector<CPoint2d> smoothpointset;

	// // smoothpointset = BSpline.GetBSPoint();

	// //CBSpline m_pathGenerR(3, temallpath);
	// //smoothpointset = m_pathGenerR.GetBSplinePoint();
	...
	// smoothpointset = LBSpline.BSplinePoints;

	

	// std::cout<<smoothpointset.size()<<std::endl;
	// CPoint2d lastpoint;
	// bool first = false;

	// for(int j=0;j<smoothpointset.size();j++)
	// {
	// 	geometry_msgs::PoseStamped tempose;
	// 	tempose.header.stamp=newpath.header.stamp;
	// 	tempose.header.frame_id=newpath.header.frame_id;
	...
	// 	if(!first)
	// 	{
	// 		std::cout<<"smoothpointset[j] "<<smoothpointset[j].x<<" "<<smoothpointset[j].y<<std::endl;
	// 		lastpoint = smoothpointset[j];
	// 		tempose.pose.position.x=smoothpointset[j].x;
	// 	 	tempose.pose.position.y=smoothpointset[j].y;
	// 	 	tempose.pose.position.z=0;
	// 	 	newpath.poses.push_back(tempose);
	// 	 	first = true;
	...
	// 	else
	// 	{
	// 		float dis = smoothpointset[j].Dist(lastpoint);
	...
	// tempose.header.stamp=newpath.header.stamp;
	// tempose.header.frame_id=newpath.header.frame_id;
	// tempose.pose.position.x=smoothpointset[smoothpointset.size()-1].x;
	// tempose.pose.position.y=smoothpointset[smoothpointset.size()-1].y;
	// tempose.pose.position.z=0;
	// newpath.poses.push_back(tempose);
