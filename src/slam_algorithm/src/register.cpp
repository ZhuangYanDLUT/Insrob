 ...
 const int imuQueLength = 200;//imu的频率，x-sens可以达到200hz，但目前我用的只能达到100hz。所以具体使用还得修改。
...
//这块包括每秒内imu的时间戳，三角数据，加速度，速度，位移。
 ...
 void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudIn2)
 {
  ...
  //把角度限定在pi-2pi之内
   ...
   //讲点云数据复制给point,注意其转向了
   /*
   x-y
   y-z
   z-x
   所以如果想在rviz下看到里程计以正常的形式跑，采集数据时，尽量以y轴向前，之后看到的里程计数据会和行走方向相同
   */
   ...
   for (int i = 0; i < cloudSize; i++)
   {
     ...
     //int scanID = int(0.5 * angle + 0.5) + 7;
		 int scanID;
     //改写算线数
     ...
     */
     ...
     //这块算了intensity，看看什么意思
     //这个值能确定该点在什么位置，第几条线的什么角度
   ...
   for (int i = 5; i < cloudSize - 5; i++) {
     ...
     if (int(laserCloud->points[i].intensity) != scanCount)
     {
       ...
       if (scanCount > 0) {
         scanStartInd[scanCount] = i + 5;//这块因为所有的点云都存在一个vector里，这里的两个值区分了每一条线点云开始索引值和结束索引值
   ...
   for (int i = 0; i < 16; i++)
   {
     ...
     for (int j = 0; j < 6; j++)
     {
       //这块是把每条线进行了分段，不过这块我觉得每条线会用到下一条线的信息（部分点包含进来），暂时我手算的是这么个情况 2016.03.02
 ...
 int main(int argc, char** argv)
 {
   ...
   // ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>("/cloud_pcd", 2, laserCloudHandler);
