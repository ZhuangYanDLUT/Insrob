 ...
 pcl::KdTreeFLANN<pcl::PointXYZINormal>::Ptr kdtreeCornerLast(new pcl::KdTreeFLANN<pcl::PointXYZINormal>());//类KdTreeFLANN是继承了kd-tree数据结构的具有3D空间检索功能实现的kdtree子类
 pcl::KdTreeFLANN<pcl::PointXYZINormal>::Ptr kdtreeSurfLast(new pcl::KdTreeFLANN<pcl::PointXYZINormal>());

 int laserCloudCornerLastNum;
 int laserCloudSurfLastNum;

 const int scanArray = 40000;

 int pointSelCornerInd[scanArray];
 float pointSearchCornerInd1[scanArray];
 float pointSearchCornerInd2[scanArray];

 int pointSelSurfInd[scanArray];
 float pointSearchSurfInd1[scanArray];
 float pointSearchSurfInd2[scanArray];
 float pointSearchSurfInd3[scanArray];
 ...
 void TransformToStart(pcl::PointXYZINormal *pi, pcl::PointXYZINormal *po)
 {
 ...
 void TransformToEnd(pcl::PointXYZINormal *pi, pcl::PointXYZINormal *po)
 {
   float s = 10 * (pi->intensity - int(pi->intensity));
 ...
 int main(int argc, char** argv)
 {
   ...
   while (status) {
     ros::spinOnce();
     //只有当这几个条件都满足时，包括有5种点，点之间的时间不超过0.005
     if (newCornerPointsSharp && newCornerPointsLessSharp && newSurfPointsFlat &&
       ...
       if (!systemInited)
       {
         ...
         kdtreeCornerLast->setInputCloud(laserCloudCornerLast);//输入点云
         ...
         pubLaserCloudCornerLast.publish(laserCloudCornerLast2);//所有的角点，16*6*20个，主角点和次角点，其实次角点包括主角点

         sensor_msgs::PointCloud2 laserCloudSurfLast2;
         pcl::toROSMsg(*laserCloudSurfLast, laserCloudSurfLast2);
         laserCloudSurfLast2.header.stamp = ros::Time().fromSec(timeSurfPointsLessFlat);
         laserCloudSurfLast2.header.frame_id = "/camera";
         pubLaserCloudSurfLast.publish(laserCloudSurfLast2);//所有所有面点，16*6*未知
       ...
       //相邻两次点云之间：角点---角线，面点---面块
       if (laserCloudCornerLastNum > 10 && laserCloudSurfLastNum > 100)
       {
         std::vector<int> indices;
         pcl::removeNaNFromPointCloud(*cornerPointsSharp,*cornerPointsSharp, indices);
         //cornerPointsSharp为新进来的角点
         ...
         for (int iterCount = 0; iterCount < 25; iterCount++)
         {
           ...
           for (int i = 0; i < cornerPointsSharpNum; i++)
           {
             ...
             if (iterCount % 5 == 0)
             {
               ...
               //上面是找到最近的点
               //pointSel为需要查询的点，k为K邻域个数
               //pointSearchInd为搜索完的邻域点对应的索引
               //pointSearchSqDis为搜索完的每个领域点与查询点索引点之间的欧式距离
             ...
             if (pointSearchCornerInd2[i] >= 0)
             {
               ...
               //下面的la,lb,lc算出来的就是，特征点指向角线的向量，的单位向量的分量，ps:真绕口
           ...
           matAtA = matAt * matA;
           matAtB = matAt * matB;
           cv::solve(matAtA, matAtB, matX, cv::DECOMP_QR);//求解线性方程组，或者最小二乘法问题
           if(std::isnan(matX.at<float>(0, 0)) || std::isnan(matX.at<float>(1, 0)) || std::isnan(matX.at<float>(2, 0)) ||
              std::isnan(matX.at<float>(3, 0)) || std::isnan(matX.at<float>(4, 0)) || std::isnan(matX.at<float>(5, 0)) )
           continue;
/*           std::cout << matX.at<float>(0, 0) << ", " << matX.at<float>(1, 0) << ", " << matX.at<float>(2, 0) << ", " 
                     << matX.at<float>(3, 0) << ", " << matX.at<float>(4, 0) << ", " << matX.at<float>(5, 0) << std::endl;
*/
//continue;
///*
           if (iterCount == 0)
...
//
           ...
           if (isDegenerate)//衰减标志？ 2016.03.08
           {
       ...
       }//需要特征点有数据，角点超过10个，平面点超过100的if判断结束位置
