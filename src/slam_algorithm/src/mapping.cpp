...
int r_w_cmd = 1;
string surf_bfile = "/home/robot/Insrob_ws/map/surf.txt";//平面点文件名
string corner_bfile = "/home/robot/Insrob_ws/map/corner.txt";//边缘点文件名
...
///*
//保存两类特征点
void build_surf_map(pcl::PointXYZINormal data, int index){
    ...
    tmp.y = data.y;
    tmp.z = data.z;
    tmp.w = data.intensity;
    
    tmp.index = index;

    fout.write((char*)&tmp ,sizeof(VPoint));
 
    fout.close();
}

void build_corner_map(pcl::PointXYZINormal data, int index){
...
//读取两类特征点文件
bool get_surf_map(){
    ifstream fin(surf_bfile.c_str(), ios::in | ios::binary);
    if(!fin.is_open())
        return false;
    VPoint tmp;
    pcl::PointXYZINormal node;
    while(fin.read((char*)&tmp, sizeof(VPoint))){
	    node.x = tmp.x;
	    node.y = tmp.y;
	    node.z = tmp.z;
	    node.intensity = tmp.w;
	    laserCloudSurfArray[tmp.index]->push_back(node);
    }
    fin.close();
    return true;
}
...
bool get_corner_map(){
  ifstream fin(corner_bfile.c_str(), ios::in | ios::binary);
    if(!fin.is_open())
        return false;
    VPoint tmp;
    pcl::PointXYZINormal node;
    while(fin.read((char*)&tmp, sizeof(VPoint))){
	    ...
	    node.z = tmp.z;
	    node.intensity = tmp.w;
	    laserCloudCornerArray[tmp.index]->push_back(node);
    }
    fin.close();
    return true;
}

bool priormapHandler(server::filename::Request &req,
				                           server::filename::Response &res)
{
    surf_bfile.clear();
    corner_bfile.clear();
       
    if("error" == req.filename)
    {
        r_w_cmd = 0;
        return false;
    }
    ...
    if("Insrob" == req.filename)
    {
      surf_bfile = "/home/robot/Insrob_ws/map/surf.txt";
      corner_bfile = "/home/robot/Insrob_ws/map/corner.txt";
    }
    ...
    if(2 == r_w_cmd)
    {
        bool surf = get_surf_map();
        bool corner = get_corner_map();
        
        if(true == corner && true == surf){
            char buf[100];
            int sum = 0;
            for(size_t ind = 0; ind < laserCloudNum;++ind)
                sum += laserCloudCornerArray[ind]->points.size() ;
    ...
    {
        res.feedback = "get prio rmap failed!";        
        return true;
    }
}
//*/
...
int main(int argc, char** argv){
   ...
   ros::ServiceServer map_name_recv = nh.advertiseService("mapname", priormapHandler);
   ...
   while (ros::ok()) {
     ...
     if (newLaserCloudCornerLast && newLaserCloudSurfLast && newLaserCloudFullRes && newLaserOdometry &&
       ...
       if (frameCount >= stackFrameNum) {
         ...
         //这块算的cubeI,cubeJ,cubeK把x,y,z裁剪在一个范围内 2016.03.24
         ...
         if(1 == r_w_cmd){
             for (int i = 0; i < laserCloudCornerStackNum; i++) {
               ...
               if (cubeI >= 0 && cubeI < laserCloudWidth &&
                 ...
                 laserCloudCornerArray[cubeInd]->push_back(pointSel);                 
                  //写边缘点文件
                  build_corner_map(pointSel, cubeInd);
               }
             }
             ...
             for (int i = 0; i < laserCloudSurfStackNum; i++) {
               ...
               if (cubeI >= 0 && cubeI < laserCloudWidth &&
                 ...
                 //写平面点文件
                 build_surf_map(pointSel, cubeInd);
               }
             }
         }
